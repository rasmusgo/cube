...
public class Search {
	...
	static String axis_name[] = {"U", "R", "F", "D", "L", "B"};
	static String power_name[] = {"00", "01", "02", "03", "10", "11", "12", "13", "20", "21", "22", "23", "30", "31", "32", "33"};
	...
	static String solutionToString(int length) {
		...
		for (int i = 0; i < length; i++) {
			s.append(axis_name[ax[i]] + power_name[po[i]] + " ");
		}
		return s.toString();
	};
	...
	public static String solution(String facelets, int maxDepth, long timeOut, boolean useSeparator) {
		...
		int mv = 1, n = 0;
		boolean busy = false; // gets set when a move is popped from the stack
		int depthPhase1 = 1;
		...
		do {
			...
			if (mv > CoordCube.N_MOVE)
				System.out.printf("n: %d ax[n]: %d po[n]: %d mv: %d ", n, ax[n], po[n], mv);
